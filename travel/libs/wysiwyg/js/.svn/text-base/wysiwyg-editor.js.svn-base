(function(window, document, $, undefined){
    ...
    var create_editor = function( $textarea, classes, placeholder, toolbar_position, toolbar_buttons, toolbar_submit, label_dropfileclick,
                                  placeholder_url, max_imagesize, onImageUpload, onKeyEnter ,booktype)
    {
        // Content: Insert link
        ...
        var content_insertimage = function(wysiwygeditor)
        {
            ...
            {
            	var curObj = wysiwygeditor.getElement();
            	var curLast = curObj.lastChild;
            	var curHeight = 0;
            	if(curLast!=null){
            		curHeight=curLast.offsetTop;
            	}
            	curHeight=curHeight+max_imagesize[1];
            	if(curHeight<=760){
	                    ...
	                    $image.css({maxWidth: max_imagesize[0]+'px',
	                          ...
	                          .load( function() {
	                                ...
	                                if( image_width > max_imagesize[0] || image_height > max_imagesize[1] )
	                                {
	                                    ...
	                                    if(image_width>530)image_width=530;
        ...
        var toolbar_button = function( button ) {
        	if(button.title=='Insert image'){
        		if(booktype=='left')
        		return $('<a/>').addClass( 'wysiwyg-toolbar-icon lanmei-image-left' )
                            .attr('href','#')
                            .attr('title', button.title)
                            .attr('id','lanmei-image-left')
                            .attr('unselectable','on')
                            .append(button.image);
                if(booktype=='right')
                return $('<a/>').addClass( 'wysiwyg-toolbar-icon lanmei-image-right' )
                            .attr('href','#')
                            .attr('title', button.title)
                            .attr('id','lanmei-image-right')
                            .attr('unselectable','on')
                            .append(button.image);
        	}else{
            ...
            $.each( toolbar_buttons, function(key, value) {
                ...
                if( 'click' in value )
                    ...
                    if(key!='fontsize'){
                		...
                		toolbar_handler = function( target ) {
		                    var curObj;
		                    if(focusCur=='left')curObj=bookLeft.getElement();
		                    if(focusCur=='right')curObj=bookRight.getElement();
			            	var curLast = curObj.lastChild;
			            	var curHeight = 0;
			            	var curWidth=0;
			            	try {
			            	...
			            	var size = 1;
			            	if($('#booksize').val()=='14px'){
			            		size=1.8;
			            	}else if($('#booksize').val()=='18px'){
			            		size=4;
			            	}else if($('#booksize').val()=='24px'){
			            		size=5;
			            	}
			            	curHeight+=size*10;
			            	if(curHeight<=760){
			            		if($('#bookclick').val()=='true'){
			            			if(focusCur=='left')bookLeft.fontSize(size);
		                    		if(focusCur=='right')bookRight.fontSize(size);
		                    		$('#bookclick').val('false');
			            		}
        ...
        if( toolbar_position != 'selection' )
        {
            ...
            var $toolbar = "";
            if(booktype=='left'){
            	$toolbar = $('<div/>').addClass( 'wysiwyg-toolbar lanmei-tollbar-left' ).addClass( toolbar_top ? 'wysiwyg-toolbar-top' : 'wysiwyg-toolbar-bottom' );
            }else if(booktype=='right'){
            	$toolbar = $('<div/>').addClass( 'wysiwyg-toolbar lanmei-tollbar-right' ).addClass( toolbar_top ? 'wysiwyg-toolbar-top' : 'wysiwyg-toolbar-bottom' );
            }else{
            	$toolbar = $('<div/>').addClass( 'wysiwyg-toolbar' ).addClass( toolbar_top ? 'wysiwyg-toolbar-top' : 'wysiwyg-toolbar-bottom' );
            }
            ...
            add_buttons_to_toolbar( $toolbar, false,
                ...
                function( $popup, target ) {
                    ...
                    if($button.context.title=='Insert image'&&$button.context.className.indexOf('left')>0){
                    	left=460;
                    	top=150;
                    }
                    if($button.context.title=='Insert image'&&$button.context.className.indexOf('right')>0){
                    	left=1100;
                    	top=150;
                    }
    ...
    $.fn.wysiwyg = function( option, param )
    {
        ...
        else if( this.length == 1 )
        {
            ...
            if( option == 'html' )
            {
            	...
            	var curObj = data.wysiwygeditor.getElement();
            	var selectedHtml = data.wysiwygeditor.getSelectedHTML();
            	var size = selectedHtml.split('</div>').length-1;
            	if(size<=0){
            		size = 1;
            	}
            	var curLast = curObj.lastChild;
            	...
            	if(curLast!=null){
            		if(curLast.offsetTop!=null){
            			...
            			if(curLast.previousSibling!=null)curHeight=curLast.previousSibling.offsetTop;
            		}
            	}
            	curHeight=curHeight+3*size;
            	
            	if(curHeight<760||curHeight<=curLast.offsetTop){
            	...
            	if(param==7)curHeight=curHeight+41*size;
            	if(param==6)curHeight=curHeight+21*size;
            	if(param==5)curHeight=curHeight+11*size;
            	if(param==4)curHeight=curHeight+4*size;
            	if(param==3)curHeight=curHeight+2*size;
            	if(param==2)curHeight=curHeight+1*size;
            	if(param==1)curHeight=curHeight+0*size;
            	if(curHeight<760||curHeight<=curLast.offsetTop){
