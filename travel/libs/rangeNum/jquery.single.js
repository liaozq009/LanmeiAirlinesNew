...
;(function($, window, document, undefined) {
	...
	jSingle.prototype = {
		...
		render: function() {
			...
			if (this.inputNode.width() === 0 && !this.options.width) {
				console.log('jSingle : no width found, returning');
				return;
			} else {
	...
	/*$.jSingle = function (node, options) {
		var jNode = $(node);
		if(!jNode.data('jSingle')){
			jNode.data('jSingle', new jSingle(node, options));
		}
		return jNode.data('jSingle');
	};

	$.fn.jSingle = function (options) {
		return this.each(function(){
			$.jSingle(this, options);
		});
	};*/

	var pluginName = 'jSingle';
	...
	$.fn[pluginName] = function(option) {
		...
		this.each(function() {
			...
			if (!data) {
				$this.data('plugin_' + pluginName, (data = new jSingle(this, options)));
