    ...
    */
    ...
    // 结束日期ID
    this.endId = this.mOpts.endId;
    // 设备
    this.device = this.mOpts.device;
    // tab
    this.mTab = this.mOpts.mTab;
    this.pTab = this.mOpts.pTab;
    //  doubleDate
        ...
        gri :[
            '<div id="' + this.calendarId + '" class="gri_dateRangeCalendar">',
                ...
                '<div class="gri_dateRangeOptions" '+ (this.mOpts.autoSubmit ? ' style="display:none" ' : '') +'>',
                    ...
                    '</div>',
                    '<div>',
                        '<input type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="OK" />',
                ...
                '<div class="ta_calendar_footer cf" '+ (this.mOpts.autoSubmit ? ' style="display:none" ' : '') +'>',
                    '<div class="frm_msg">',
                        '<div id="' + this.dateRangeDiv + '">',
                            '<input type="text" class="ta_ipt_text_s" name="' + this.startDateId + '" id="' + this.startDateId + '" value="' + this.mOpts.startDate + '" style="text-align:right" readonly />',
                            '<span class="' + this.mOpts.joinLineId + '"> - </span>',
                    ...
                    '</div>',
                    '<div class="frm_btn">',
                        '<input class="ta_btn ta_btn_primary" type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="OK" />',
    ...
    if(this.device=='mobile'){
        $("#"+ this.inputId).bind('click', function() {
            ...
            var inputId = __method.inputId;

            if(__method.doubleDate){ //显示往返日期
                var sTime = $('#' + __method.startDateId).val();
                var eTime = $('#' + __method.endDateId).val();
                
                $('#m-hotel-selectTime').val(sTime+' ~ '+eTime); //首页输入框显示

                $('#'+__method.startDateId).val(sTime);
                $('#'+__method.endDateId).val(eTime);

                __method.mOpts.isSingleDay=false;
                __method.mOpts.shortOpr=false;
                __method.addCSS(0);

                __method.show(false, __method);
            }else{
                if(inputId=='m-ticket-selectTime'){
                    for(var i=0; i<$(__method.mTab).length; i++){
                        if($($(__method.mTab)[i]).hasClass('active')){
                            if($($(__method.mTab)[i]).attr('data-way')=='oneWay'){
                                __method.mOpts.isSingleDay=true;
                                __method.mOpts.shortOpr=true;
                                __method.selectDate($("#"+ __method.inputId).val()); //显示日期背景颜色,根据首页日期输入框的值显示
                                
                                __method.show(false, __method); //显示日期界面
        ...
        // tab切换，只应用与ticket
        $(this.mTab).click(function(){
            var val = $(this).attr('data-way');

            if(val=='oneWay'){
               $("#m-ticket-selectTime").val($('#' + __method.mOpts.startDateId).val()); //只等于日期左下角开始时间的值
                
               $('#'+__method.startDateId).val($('#' + __method.mOpts.startDateId).val());
               $('#'+__method.endDateId).val($('#' + __method.mOpts.startDateId).val());
               ...
               __method.mOpts.shortOpr=true;
                ...
                var sTime = $('#' + __method.mOpts.startDateId).val();

                var endTimeStr = new Date(new Date(Date.parse(sTime.replace(/-/g,  "/"))).getTime()+86400000*2);
                 
                var eTime = __method.formatDate(endTimeStr.getFullYear()+'-'+(endTimeStr.getMonth()+1)+'-'+(endTimeStr.getDate()));

                $('#m-ticket-selectTime').val(sTime+' ~ '+eTime);
        ...
        $("#"+ this.inputId).bind('click', function() {
            ...
            if(__method.doubleDate){ //显示往返日期
                ...
                if(inputId=='p-ticket-selectTime'){
                    for(var i=0; i<$(__method.pTab).length; i++){
                        if($($(__method.pTab)[i]).children('i').hasClass('active')){
                            if($($(__method.pTab)[i]).children('span').attr('data-way')=='oneWay'){
                                __method.mOpts.isSingleDay=true;
                                __method.mOpts.shortOpr=true;
                                __method.selectDate($("#"+ __method.inputId).val()); //显示日期背景颜色,根据首页日期输入框的值显示
        ...
        $(this.pTab).click(function(){
            var val = $(this).children('span').attr('data-way');

            if(val=='oneWay'){
                $("#p-ticket-selectTime").val($('#' + __method.mOpts.startDateId).val()); //只等于日期左下角开始时间的值
               ...
               __method.mOpts.shortOpr=true;
                ...
                $('#p-ticket-selectTime').val(sTime+' ~ '+eTime);
    ...
    if(this.doubleDate){
        ...
        var eTime = this.formatDate(endTimeStr.getFullYear()+'-'+(endTimeStr.getMonth()+1)+'-'+(endTimeStr.getDate()));

        $("#"+ this.inputId).val(sTime+' ~ '+eTime);
    }
...
pickerDateRange.prototype.init = function(isCompare) {
    ...
    var endDate = '' == this.mOpts.endDate ? (new Date()) : this.str2date(this.mOpts.maxTime);
    // 日历结束时间
...
pickerDateRange.prototype.selectDate = function(ymd) {
    ...
    // 转化ymd的值
    var ymd = ymd.replace(/(\d{4})\-(\d{1,2})\-(\d{1,2})/g, function(ymdFormatDate, y, m, d){
        m = parseInt(m);
        d = parseInt(d);
    ...
    if(this.startDateId == this.dateInput) {
        ...
        if (true == this.mOpts.singleCompare || true == this.mOpts.isSingleDay) {
            ...
            (this.mOpts.autoSubmit) && this.close(1);
...
pickerDateRange.prototype.show = function(isCompare, __method) {
    ...
    if (true == this.mOpts.singleCompare || true == this.mOpts.isSingleDay) {
        ...
        $('.' + this.mOpts.joinLineId).css('display', 'none');
         ...
         $('.joinLine').css('display', 'inline-block');
         $('#' + this.endDateId).css('display', 'inline-block');
         
         $('#' + __method.mOpts.startDateId).val(__method.mOpts.startDate);
         $('#' + __method.mOpts.endDateId).val(__method.mOpts.endDate);
    ...
    if(btnSubmit){
        ...
        if (this.mOpts.shortOpr === true){
            if(this.device=='mobile'){
                ...
                $('#' + this.inputId).val($('#' + this.startDateId).val());
                // $('#' + this.endId).attr("disabled",true);
            }
            
            // console.log($('#' + this.inputId).val());
            $('#' + this.inputCompareId).val($('#' + this.startCompareDateId).val());
        }else{
            var _val = $('#' + this.startDateId).val() + ('' == $('#' + this.endDateId).val() ? '' : this.mOpts.defaultText + $('#' + this.endDateId).val());

            // $('#' + this.inputId).val($('#' + this.startDateId).val() + ('' == $('#' + this.endDateId).val() ? '' : this.mOpts.defaultText + $('#' + this.endDateId).val()));
            // if(this.device=='mobile'){
                $('#' + this.inputId).val(_val);
            // }else{
            //     var timeStr = _val.split('~');
            //     var startStr = timeStr[0];
            //     var endStr = timeStr[1];

            //     $('#' + this.inputId).val(startStr);
            //     $('#' + this.endId).val(endStr);
            // }
        }
        ...
        if(input && input.tagName == 'INPUT'){
            ...
            //     $('#' + this.inputId).html(startStr);
            //     $('#' + this.endId).html(endStr);
            // }
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    if(isTaTheme){
        ...
        if(this.mOpts.magicSelect){
            ...
            yh.push('</ul></div></div>');
            mh.push('</ul></div></div>');           
            $(cap).append(yh.join('') +'<span class="joinLine"> - </span>'+mh.join('')+'<span class="joinLine"> </span>');
    ...
    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期 1497628800000   1497665970573 Sat Jun 17 2017 00:00:00 GMT+0800 (中国标准时间)
            deviation = '0';
