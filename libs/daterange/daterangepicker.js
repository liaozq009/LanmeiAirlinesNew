...
(function (root, factory) {
    ...
    var DateRangePicker = function(element, options, cb) {
        ...
        // 全局定义酒店入住了多少晚
        this.showTotleDay = $.extend({showTotleDay:false}, {showTotleDay:options.showTotleDay}).showTotleDay;
        var startNight = 1;
        var endNight = 1;
        var totleNight = 0;
        ...
        if (this.singleDatePicker) {
            ...
            this.container.find('.daterangepicker_input input, .daterangepicker_input > i').hide(); //隐藏动态input选择的日期
        }
        ...
        if (this.element.is('input') || this.element.is('button') || this.element.is('span')) { //防止重复点击出发地而隐藏日期控件
            this.element.on({
        ...
        //
        // if attached to a text input, set the initial value 初始化日期
        //
        if (this.element.is('input') && !this.singleDatePicker && this.autoUpdateInput) {
            this.element.val(this.startDate.format(this.locale.format) + this.locale.separator + this.endDate.format(this.locale.format));
            this.element.trigger('change');
        }else if(!this.element.is('input') && !this.singleDatePicker && this.autoUpdateInput){
            this.element.html(this.startDate.format(this.locale.format) + this.locale.separator + this.endDate.format(this.locale.format));
            this.element.trigger('change');
        }else if (this.element.is('input') && this.autoUpdateInput) { //单程日期
    ...
    DateRangePicker.prototype = {
        ...
        renderCalendar: function(side) {
            ...
            if ((!minDate || minDate.isBefore(calendar.firstDay)) && (!this.linkedCalendars || side == 'left')) {
                html += '<th class="prev available"><i class="fa fa-' + arrow.left + ' glyphicon glyphicon-date-prev"></i></th>';
            } else {
                html += '<th></th>';
            }
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    // 添加指定日期删除和价格
                    var valueOf = calendar[row][col].valueOf();
                    var price = '-';
                    if (valueOf==1520006400000){
                        classes.push('off', 'disabled');
                        price = '￥99';
                    }
                    ...
                    if (!disabled)
                        cname += 'available';
                    // html += '<td class="' + cname.replace(/^\s+|\s+$/g, '') + '" data-title="' + 'r' + row + 'c' + col + '">' + calendar[row][col].date() + '<span style="display:block;">'+price+'</span></td>';
            ...
            if(this.showTotleDay){
                if (this.endDate && !this.container.find('input[name=daterangepicker_start]').is(":focus")) {
                    ...
                    if(endNight<startNight){
                        startNight = this.startDate.toDate().getTime(); //防止鼠标往后滑动产生的bug
                        totleNight = 0;
                    }
                    this.container.find('.totle-num-date').html(totleNight);
                    $('.hotel-day').html(totleNight);
